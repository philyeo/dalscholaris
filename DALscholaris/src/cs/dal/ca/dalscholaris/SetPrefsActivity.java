...
public class SetPrefsActivity extends PreferenceActivity implements OnSharedPreferenceChangeListener {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		//showAuthorPref = (CheckBoxPreference)getPreferenceScreen().findPreference("showAuthor");
		bioPref = (CheckBoxPreference)getPreferenceScreen().findPreference("bio");
		busPref = (CheckBoxPreference)getPreferenceScreen().findPreference("bus");
		chmPref = (CheckBoxPreference)getPreferenceScreen().findPreference("chm");
		engPref = (CheckBoxPreference)getPreferenceScreen().findPreference("eng");
		medPref = (CheckBoxPreference)getPreferenceScreen().findPreference("med");
		phyPref = (CheckBoxPreference)getPreferenceScreen().findPreference("phy");
		socPref = (CheckBoxPreference)getPreferenceScreen().findPreference("soc");

		homepagePref = (EditTextPreference)getPreferenceScreen().findPreference("homePage");

		proxyPref = (CheckBoxPreference)getPreferenceScreen().findPreference("enableProxy");
		hostPref = (EditTextPreference)getPreferenceScreen().findPreference("proxyHost");
		portPref = (EditTextPreference)getPreferenceScreen().findPreference("proxyPort");
		usernamePref = (EditTextPreference)getPreferenceScreen().findPreference("username");
		passwordPref = (EditTextPreference)getPreferenceScreen().findPreference("password");
		...
		customColorPref = (CheckBoxPreference)getPreferenceScreen().findPreference("useCustomColor");
		bgColorPref = findPreference("bgColor");
		bgColorPref.setOnPreferenceClickListener(mColorchangeListener);
	...
	public void onSharedPreferenceChanged(SharedPreferences sharedPreferences, String key) {     
		...
		if (key.equalsIgnoreCase("startYear")) {         			       
		...
		else if (key.equalsIgnoreCase("downloadPath")) {
			...
			if(checkValidFoldername(filelocation)) {
				...
				Toast.makeText(SetPrefsActivity.this, "Invalid folder name.", Toast.LENGTH_LONG).show();
			}
		}
		...
		else if (key.equalsIgnoreCase("homePage")) {         			       
			homepagePref.setSummary(homepagePref.getText());  			
		}
		else if (key.equalsIgnoreCase("useCustomColor")) {         			       
			if(customColorPref.isChecked()) 
				bgColorPref.setEnabled(true);
			else
				bgColorPref.setEnabled(false);
		}
		else if (key.equalsIgnoreCase("bgColor")) { 
			String strselectedColor = sharedPreferences.getString("bgColor","#000000");
			...
			bgColorPref.setSummary(summary);
						
		}
		else if (key.equalsIgnoreCase("enableProxy")) {         			       
			if(proxyPref.isChecked()) {
				//if(D) Log.d("MyApp","Host="+hostPref.getText());
				hostPref.setEnabled(true);
				hostPref.setSummary(hostPref.getText());
				portPref.setEnabled(true);
		...
		else if (key.equalsIgnoreCase("proxyHost")) {         			       
			hostPref.setSummary(hostPref.getText());  			
		}
		else if (key.equalsIgnoreCase("proxyPort")) {         			       
			portPref.setSummary(portPref.getText());  			
		}
	...
	protected void onResume() {         
		...
		summary = new SpannableString(appPref.getString("bgColor","#123456")); 
		summary.setSpan(new ForegroundColorSpan(Color.parseColor(summary.toString())), 0, summary.length(), 0 ); 
    	bgColorPref.setSummary(summary);
		//showAuthorPref.setChecked(showAuthorPref.isChecked());
	...
	public boolean onMenuItemSelected(int featureId, MenuItem item) {
		switch(item.getItemId()) {
			...
			case R.id.clearhistory:	AlertDialog.Builder builder = new AlertDialog.Builder(SetPrefsActivity.this);
									...
									builder.setPositiveButton("Yes", new DialogInterface.OnClickListener() {
										...
										public void onClick(DialogInterface dialog, int which) {
											...
											Toast.makeText(SetPrefsActivity.this, "Search history has been cleared.", Toast.LENGTH_LONG).show();
										}
									});
									...
									builder.setNegativeButton("No", new DialogInterface.OnClickListener() {
										...
										public void onClick(DialogInterface dialog, int which) {
											...
											Toast.makeText(SetPrefsActivity.this, "No changes were made to search history.", Toast.LENGTH_LONG).show();
											dialog.cancel();
										}
									});
			...
			case R.id.returntosearch:	SetPrefsActivity.this.finish();
		...
		builder.setNegativeButton("Delete", new DialogInterface.OnClickListener() {
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (mJournalList.length>0) {
					...
					Toast.makeText(SetPrefsActivity.this, "Journal name \""+chosenJournal+"\" has been deleted", Toast.LENGTH_SHORT).show();
				}
			...
			public void onClick(DialogInterface dialog, int which) {
				...
				if (!journal.toString().equalsIgnoreCase("")) {
					...
					if(!journalPreference.contains(journal.toString())) {
						...
						Toast.makeText(SetPrefsActivity.this, "The journal name "+"\""+journal.toString().toString()+"\" has been saved.", Toast.LENGTH_SHORT).show();
					}
					else
						Toast.makeText(SetPrefsActivity.this, "The journal name "+"\""+journal.toString().toString()+"\" already exists.", Toast.LENGTH_SHORT).show();
				}
				else
					Toast.makeText(SetPrefsActivity.this, "Invalid name.", Toast.LENGTH_SHORT).show();
